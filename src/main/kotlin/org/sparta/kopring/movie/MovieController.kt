...
class MovieController(
    ...
    fun getMovie(@PathVariable movieId: Long): MovieResponse {
        return movieService.getMovie(movieId)
    }

    @PostMapping("/api/v1/movies")
    fun saveMovie(@RequestBody movieRequest: MovieRequest) {
        movieService.saveMovie(movieRequest)
    }

    @PostMapping("/api/v1/movies/bulk")
    suspend fun saveMovieBulkAsync(@RequestBody movieRequest: MovieRequest) {
